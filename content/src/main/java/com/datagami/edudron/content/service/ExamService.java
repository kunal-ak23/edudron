...
public class ExamService {
                        ...
                        if (attributes != null) {
                            ...
                            if (authHeader != null && !authHeader.isBlank()) {
                                ...
                                if (!request.getHeaders().containsKey("Authorization")) {
                                    ...
                                    logger.debug("Propagated Authorization header (JWT token) to identity service: {}",
                                            request.getURI());
                                } else {
                                    logger.debug("Authorization header already present in request to {}",
                                            request.getURI());
                                }
                            } else {
    ...
    public Assessment createExam(String courseId, String title, String description, String instructions,
        ...
        if ("INSTRUCTOR".equals(userRole) || "SUPPORT_STAFF".equals(userRole) || "STUDENT".equals(userRole)) {
            throw new IllegalArgumentException(
                    "INSTRUCTOR, SUPPORT_STAFF, and STUDENT have view-only access and cannot create exams");
        }
    ...
    public Assessment scheduleExam(String examId, OffsetDateTime startTime, OffsetDateTime endTime) {
        ...
        if ("INSTRUCTOR".equals(userRole)) {
            if (!canInstructorManageExam(exam)) {
                throw new IllegalArgumentException(
                        "You don't have permission to adjust the schedule for this exam. It's outside your assigned scope.");
            }
        } else if ("SUPPORT_STAFF".equals(userRole) || "STUDENT".equals(userRole)) {
            throw new IllegalArgumentException(
                    "SUPPORT_STAFF and STUDENT have view-only access and cannot schedule exams");
        }
     ...
     * Uses /me/access (same as exam list pagination); non-instructors or failed
     * access see all exams.
     */
    private List<Assessment> filterExamsForInstructor(List<Assessment> exams) {
        ...
        logger.info(
                "Instructor exam filter: allowedCourseIds={} (sample: {}), allowedClassIds={} (sample: {}), allowedSectionIds={} (sample: {}), sectionOnlyAccess={}, examsBefore={}",
    ...
    public Page<Assessment> getExamsPaginated(String status, String timingMode, String search, int page, int size) {
        ...
        // Use instructor access (same as courses page) to decide filtering, not
        // identity role.
        // This way we only call the student service; no dependency on /idp/users/me for
        // role.
        // If /me/access returns a body with any allowed IDs, treat as instructor and
        // filter.
        Map<String, Object> instructorAccess = getCurrentUserInstructorAccess();
        ...
        if (instructorAccess != null) {
            ...
            logger.info(
                    "Exam list: no instructor access (null); returning all exams (non-instructor or /me/access failed)");
        }
    ...
    /**
     * Effective allowed course IDs: allowedCourseIds, or union of directCourseIds +
     * derivedCourseIds when empty.
     * Handles case where student service returns empty allowed* but populated
     * direct/derived.
     */
    ...
    /**
     * Effective allowed class IDs: allowedClassIds, or union of directClassIds +
     * inheritedClassIds when empty.
     */
    ...
    /**
     * Effective allowed section IDs: allowedSectionIds, or union of
     * directSectionIds + inheritedSectionIds when empty.
     */
    ...
    /**
     * Get paginated exams for an instructor, filtered by their section/class/course
     * assignments.
     * 
     * @param instructorAccess already-fetched access (from
     *                         getCurrentUserInstructorAccess); must be non-null.
     */
    private Page<Assessment> getExamsPaginatedForInstructor(UUID clientId, Assessment.ExamStatus examStatus,
        ...
        boolean allEmpty = allowedCourseIds.isEmpty() && allowedClassIds.isEmpty() && allowedSectionIds.isEmpty();
        logger.info(
                "Instructor pagination: allowedCourseIds={} (sample: {}), allowedClassIds={} (sample: {}), allowedSectionIds={} (sample: {}), sectionOnlyAccess={}",
                allowedCourseIds.size(), sampleIds(allowedCourseIds, 5),
                allowedClassIds.size(), sampleIds(allowedClassIds, 5),
                allowedSectionIds.size(), sampleIds(allowedSectionIds, 5), sectionOnlyAccess);
        if (allEmpty) {
            logger.warn(
                    "Instructor pagination: all allowed* sets are empty. Instructor has no assignments or /me/access returned empty. No exams will be returned.");
        }
        ...
        // Filter by instructor access (section-only instructors see only section-level
        // exams)
        ...
        if (totalFetched > 0 && filteredExams.isEmpty() && !allEmpty) {
            // Have exams and have allowed IDs but none matched - log first exam's IDs for
            // debugging
            Assessment first = allExams.getContent().get(0);
            logger.warn(
                    "Instructor pagination: no exam matched. First exam: courseId={}, classId={}, sectionId={} (normalized: {}, {}, {})",
                    first.getCourseId(), first.getClassId(), first.getSectionId(),
                    normalizeId(first.getCourseId()), normalizeId(first.getClassId()),
                    normalizeId(first.getSectionId()));
        }
    ...
    public Map<String, Long> getExamCountsByStatus() {
        ...
        // Use instructor access (same as exam list) to decide; don't depend on identity
        // role
        Map<String, Object> instructorAccess = getCurrentUserInstructorAccess();
        if (instructorAccess != null && hasAnyAllowedIds(instructorAccess)) {
    ...
    /**
     * Get exam counts by status for an instructor, filtered by their
     * section/class/course assignments.
     * 
     * @param instructorAccess already-fetched access (caller ensures non-null and
     *                         has allowed IDs).
     */
    private Map<String, Long> getExamCountsByStatusForInstructor(UUID clientId, Map<String, Object> instructorAccess) {
        ...
        logger.info(
                "Instructor exam counts: allowedCourseIds={}, allowedClassIds={}, allowedSectionIds={}, sectionOnlyAccess={}",
                allowedCourseIds.size(), allowedClassIds.size(), allowedSectionIds.size(), sectionOnlyAccess);

        // Fetch all exams and filter by instructor access (section-only: only
        // section-level exams)
        ...
        for (Assessment exam : allExams) {
            if (canInstructorAccessExam(exam, allowedCourseIds, allowedClassIds, allowedSectionIds,
                    sectionOnlyAccess)) {
                String statusName = exam.getStatus() != null ? exam.getStatus().name()
                        : Assessment.ExamStatus.DRAFT.name();
    ...
    /**
     * Get exam detail as DTO (cached in Redis). Same exam for many students = one
     * DB hit per tenant.
     */
    ...
    /**
     * Evict cached exam detail after any mutation (update, schedule, publish,
     * question change, etc.).
     */
    ...
    /**
     * Evict cached exam detail by exam id and client id (for use when tenant
     * context is not set, e.g. scheduled job).
     */
    ...
    public Assessment updateExam(String examId, String title, String description, String instructions,
        ...
        if ("INSTRUCTOR".equals(userRole) || "SUPPORT_STAFF".equals(userRole) || "STUDENT".equals(userRole)) {
            throw new IllegalArgumentException(
                    "INSTRUCTOR, SUPPORT_STAFF, and STUDENT have view-only access and cannot update exams");
        }
    ...
    public boolean deleteExam(String examId) {
        ...
        if ("INSTRUCTOR".equals(userRole) || "SUPPORT_STAFF".equals(userRole) || "STUDENT".equals(userRole)) {
            throw new IllegalArgumentException(
                    "INSTRUCTOR, SUPPORT_STAFF, and STUDENT have view-only access and cannot delete exams");
        }
    ...
    public Assessment publishExam(String examId) {
        ...
        if ("SUPPORT_STAFF".equals(userRole) || "STUDENT".equals(userRole)) {
            throw new IllegalArgumentException(
                    "SUPPORT_STAFF and STUDENT have view-only access and cannot publish exams");
        }
        ...
        if ("INSTRUCTOR".equals(userRole)) {
            if (!canInstructorManageExam(exam)) {
                throw new IllegalArgumentException(
                        "You don't have permission to publish this exam. It's outside your assigned scope.");
            }
        }
        ...
        if (exam.getTimingMode() == Assessment.TimingMode.FIXED_WINDOW) {
            if (exam.getStartTime() == null || exam.getEndTime() == null) {
                throw new IllegalStateException(
                        "Start and end times must be set for Fixed Window exams before publishing");
            }
    ...
    public Assessment unpublishExam(String examId) {
        ...
        if ("SUPPORT_STAFF".equals(userRole) || "STUDENT".equals(userRole)) {
            throw new IllegalArgumentException(
                    "SUPPORT_STAFF and STUDENT have view-only access and cannot unpublish exams");
        }
        ...
        if ("INSTRUCTOR".equals(userRole)) {
            if (!canInstructorManageExam(exam)) {
                throw new IllegalArgumentException(
                        "You don't have permission to unpublish this exam. It's outside your assigned scope.");
            }
        }
    ...
    public Assessment completeExam(String examId) {
        ...
        if ("SUPPORT_STAFF".equals(userRole) || "STUDENT".equals(userRole)) {
            throw new IllegalArgumentException(
                    "SUPPORT_STAFF and STUDENT have view-only access and cannot complete exams");
        }
        ...
        if ("INSTRUCTOR".equals(userRole)) {
            if (!canInstructorManageExam(exam)) {
                throw new IllegalArgumentException(
                        "You don't have permission to complete this exam. It's outside your assigned scope.");
            }
        }
    ...
    /**
     * Republish a completed exam (make it live/available to students again).
     * Only SYSTEM_ADMIN and TENANT_ADMIN can republish completed exams.
     * For FLEXIBLE_START: immediately goes LIVE.
     * For FIXED_WINDOW: re-evaluates the window; SCHEDULED if start is in the
     * future,
     * LIVE if within the window, error if the end time has already passed.
     */
    public Assessment republishExam(String examId) {
        String userRole = getCurrentUserRole();

        // Only SYSTEM_ADMIN and TENANT_ADMIN can republish completed exams
        if (!"SYSTEM_ADMIN".equals(userRole) && !"TENANT_ADMIN".equals(userRole)) {
            throw new IllegalArgumentException("Only SYSTEM_ADMIN and TENANT_ADMIN can republish completed exams");
        }
        ...
        if (exam.getStatus() != Assessment.ExamStatus.COMPLETED) {
            throw new IllegalStateException(
                    "Only COMPLETED exams can be republished. Current status: " + exam.getStatus());
        }

        if (exam.getTimingMode() == Assessment.TimingMode.FIXED_WINDOW) {
            if (exam.getStartTime() == null || exam.getEndTime() == null) {
                throw new IllegalStateException(
                        "Start and end times must be set for Fixed Window exams. Please set a schedule first.");
            }

            OffsetDateTime now = OffsetDateTime.now();
            if (now.isAfter(exam.getEndTime())) {
                throw new IllegalStateException(
                        "Cannot republish a Fixed Window exam whose end time has already passed. Please reschedule the exam first.");
            } else if (now.isBefore(exam.getStartTime())) {
                exam.setStatus(Assessment.ExamStatus.SCHEDULED);
                logger.info("Exam {} republished as SCHEDULED (starts in future)", examId);
            } else {
                exam.setStatus(Assessment.ExamStatus.LIVE);
                logger.info("Exam {} republished as LIVE (within window)", examId);
            }
        } else {
            // FLEXIBLE_START — immediately go live
            exam.setStatus(Assessment.ExamStatus.LIVE);
            logger.info("Exam {} (Flexible Start) republished as LIVE", examId);
        }
    ...
    public void archiveExam(String examId) {
        ...
        if ("INSTRUCTOR".equals(userRole) || "SUPPORT_STAFF".equals(userRole) || "STUDENT".equals(userRole)) {
            throw new IllegalArgumentException(
                    "INSTRUCTOR, SUPPORT_STAFF, and STUDENT have view-only access and cannot archive exams");
        }
    ...
    public void unarchiveExam(String examId) {
        ...
        if ("INSTRUCTOR".equals(userRole) || "SUPPORT_STAFF".equals(userRole) || "STUDENT".equals(userRole)) {
            throw new IllegalArgumentException(
                    "INSTRUCTOR, SUPPORT_STAFF, and STUDENT have view-only access and cannot unarchive exams");
        }
    ...
    /**
     * Batch generate exams for multiple sections with randomized question
     * selection.
    ...
    private Assessment createExamForBatch(BatchExamGenerationRequest request, String sectionId, String title,
            UUID clientId) {
        // Get next sequence
        Integer maxSequence = assessmentRepository.findMaxSequenceByCourseIdAndClientId(request.getCourseId(),
                clientId);
        int nextSequence = (maxSequence != null ? maxSequence : 0) + 1;
        ...
        if (settings != null) {
            ...
            exam.setPassingScorePercentage(
                    settings.getPassingScorePercentage() != null ? settings.getPassingScorePercentage() : 70);
            ...
            exam.setPhotoIntervalSeconds(
                    settings.getPhotoIntervalSeconds() != null ? settings.getPhotoIntervalSeconds() : 120);
            ...
            exam.setMaxTabSwitchesAllowed(
                    settings.getMaxTabSwitchesAllowed() != null ? settings.getMaxTabSwitchesAllowed() : 3);
    ...
    private int generateQuestionsForExam(String examId, BatchExamGenerationRequest request, UUID clientId) {
        ...
        if (criteria != null) {
            genCriteria.setNumberOfQuestions(
                    criteria.getNumberOfQuestions() != null ? criteria.getNumberOfQuestions() : 10);
     ...
     * GET /api/instructor-assignments/instructor/{userId}/access so we do not rely
     * on
    ...
    private Map<String, Object> getCurrentUserInstructorAccess() {
        ...
        if (access != null) {
            logger.info(
                    "Instructor access OK: userId={}. allowedCourseIds size: {}, allowedClassIds size: {}, allowedSectionIds size: {}",
                    userId, listSize(access.get("allowedCourseIds")), listSize(access.get("allowedClassIds")),
                    listSize(access.get("allowedSectionIds")));
        }
        return access;
    }
     ...
     * Uses typed DTO so Jackson deserializes JSON arrays as List&lt;String&gt;
     * (avoids
     * Map&lt;String, Object&gt; type ambiguity that can yield size 0). Forwards
     * X-Client-Id
     * and Authorization so the student service sees the same tenant and auth as the
     * browser.
     * Returns null if unable to determine access.
     */
    private Map<String, Object> getInstructorAccess(String instructorUserId) {
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                ...
                logger.info(
                        "Instructor access response (content service): url={}, instructorUserId={}, allowedCourseIds size={}, allowedClassIds size={}, allowedSectionIds size={}, sectionOnlyAccess={}",
                        accessUrl,
    ...
    /**
     * Return size of a list/collection/array for logging (0 if null or not a
     * list/array).
    ...
    /**
     * Normalize ID for comparison (trim, lowercase) so UUID/string format
     * differences match.
     */
    ...
    /**
     * Helper method to extract a Set of normalized strings from instructor access
     * response.
     * Handles List/Set/array and converts elements to String (handles Number, UUID,
     * etc. from JSON).
     * Tries exact key first, then case-insensitive match. Handles JSON arrays
     ...
     * When sectionOnlyAccess is true, only section-level exams (exam.sectionId set)
     * in allowedSectionIds are shown.
     */
    private boolean canInstructorAccessExam(Assessment exam,
        ...
        // Instructor with only section-level access: show only section-level exams
