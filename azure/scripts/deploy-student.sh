...
# Deploy Student Service to Azure Container Apps
# Usage: ./azure/scripts/deploy-student.sh --config <config-file-path>
...
# Get service config
PORT=$(jq -r ".services.${SERVICE}.port" "$SERVICES_CONFIG" 2>/dev/null || echo "8083")
