...
# Deploy Core API Service to Azure Container Apps
# Bundled service: identity + student + payment
# Usage: ./azure/scripts/deploy-core-api.sh --config <config-file-path>
...
# Get service config
PORT=$(jq -r ".services.\"${SERVICE}\".port" "$SERVICES_CONFIG" 2>/dev/null || echo "8085")
...
print_info "Deploying ${SERVICE} service (bundled: identity + student + payment)"
print_info "  Version: ${VERSION}"
print_info "  Image: ${IMAGE}"
print_info "  App Name: ${APP_NAME}"
print_info "  Resource Group: ${RESOURCE_GROUP}"
print_info "  Resources: ${CPU} CPU, ${MEMORY} memory"
...
if [ -z "$APP_EXISTS" ]; then
    ...
    # Register secrets — union of identity + student + payment secrets
    ...
    # Set environment variables — merged from identity + student + payment
...
print_info "Health check: az containerapp exec --name $APP_NAME --resource-group $RESOURCE_GROUP --command 'curl -s http://localhost:${PORT}/actuator/health'"
